...
public class LockHttpAction extends BaseHttpAction {
    ...
    private LockHttpAction(Context context) {
        ...
        CLIENT_ID = SharePreUtil.getInstance(context).getString(Constant.CLIENT_ID, "");
        CLIENT_SECRET = SharePreUtil.getInstance(context).getString(Constant.CLIENT_SECRET, "");
    }
    ...
    public static LockHttpAction getHttpAction(Context context) {
        ...
        ACCESS_TOKEN = SharePreUtil.getInstance(context).getString(Constant.ACCESS_TOKEN, "");
        return sHttpAction;
    }

    public void regist(String userName, String pwd, OnHttpRequestCallback<Boolean> listener) {
        Log.e("TAG","-----------regist:"+CLIENT_ID+"::"+CLIENT_SECRET+"::"+userName+pwd);
        mHttpRequest.regist(CLIENT_ID, CLIENT_SECRET, userName, DigitUtil.getMD5(pwd), currentTimeMillis(), listener);
    }

    public void auth(String userName, String pwd, String directUri, final OnHttpRequestCallback<LockUser> listener) {
        mHttpRequest.auth(CLIENT_ID, CLIENT_SECRET, userName, "password", pwd, directUri, new OnHttpRequestCallback<LockUser>() {
            @Override
            ...
            public void onSuccess(LockUser lockUser) {
                if (listener != null)
                    listener.onSuccess(lockUser);
                SharePreUtil.getInstance(sContext).setString(Constant.ACCESS_TOKEN, lockUser.getAccess_token());
            }
        });
    }
     ...
     * 获取ic卡列表
     */
    public void getICs(int lockId, int pageNo, int pageSize, OnHttpRequestCallback<HttpResult<ICard>> listener) {
     ...
     * 获取指纹列表
     */
    public void getFingerprints(int lockId, int pageNo, int pageSize, OnHttpRequestCallback<HttpResult<LockFingerprint>> listener) {
